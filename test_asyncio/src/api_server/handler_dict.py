from ..msg import msg_packet_api_pb2
from ..msg import msg_type_api_pb2


PACKET_CLASS_TYPE_DICT = {}
for i in dir(msg_packet_api_pb2):
    find_name = 't_' + i
    for j in dir(msg_type_api_pb2):
        if find_name == j:
            PACKET_CLASS_TYPE_DICT[getattr(msg_packet_api_pb2, i)] = getattr(msg_type_api_pb2, find_name)
            break
...
for i in PACKET_CLASS_TYPE_DICT.keys():
    ...
    for j in dir(msg_packet_api_pb2):
        if find_name == j:
            PACKET_REQ_ACK_DICT[i] = getattr(msg_packet_api_pb2, find_name)
            break
